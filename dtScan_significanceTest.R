...
for (trgt in targetList) {
  ...
  for (drvr in driverList) {
    ...
    resDrvrPlt$driver <- factor(resDrvrPlt$driver,
                                levels = sort(as.vector(unique(resDrvrPlt$driver))))
    ...
    if (sigPlot == T) {
      ...
      for (dtt in 1:length(dt_c)) {
        ...
        for (mm in 1:length(m_c)) {
          ...
          # resTest_org <- as.data.frame(coredata(resTest_org)[rep(1:nrow(resTest_org), each=10), ])
          
          ### significance test between original and surrogate estimates
          if (sigMethod == "tTest") {
            
            # diff <- resTest_org$TE - resTest_sur$TE
            # shapTest <- shapiro.test(diff)
            pList <- c()
    ...
    if (sumData == T) {
      ...
      if (sigPlot == T) {
        pl <- pl +
          geom_text_repel(aes(x = (d_t + d_t)/1.7, y = mean + 0.0, 
